...
namespace AdmiralFive
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            Console.Write("simulation sweeping victory: ");
            if (sim.SweepingVictory(Fraction.Separatists) == null &&
                    sim.SweepingVictory(Fraction.Republic) == null)
                Console.WriteLine("OK");
            else { Console.WriteLine("WRONG"); }


            sim.NewBattle(Fraction.Separatists, p, starC, corv2);
            Console.Write("simulation sweeping victory 2: ");
            if (sim.SweepingVictory(Fraction.Separatists) == null &&
                    sim.SweepingVictory(Fraction.Republic) == null)
                Console.WriteLine("OK");
            else { Console.WriteLine("WRONG"); }
            ...
            try
            {
                Console.Write("Simulation new battle: ");
                sim.NewBattle(Fraction.Separatists, p, starC, corv);
                Console.WriteLine("WRONG");
            }
            catch (Exception)
            {
            ...
            Console.Write("simulation destoyedships: ");
            if (sim.DestroyedShips("planet1") == 0) Console.WriteLine("OK");
            else { Console.WriteLine("WRONG"); }

            Console.Write("simulation sweeping victory 3: ");
            Planet p2 = new Planet("planet2", 2);
            sim.NewBattle(Fraction.Republic, p2, corv, corv2);
            if (sim.SweepingVictory(Fraction.Separatists) == p2 &&
                    sim.SweepingVictory(Fraction.Republic) == null)
                Console.WriteLine("OK");
            else { Console.WriteLine("WRONG"); }

            Console.Write("simulation sweeping victory 4: ");
            sim.Reinforcement(Fraction.Separatists, p, new List<Ship> { starC });
            sim.Run(10);
            if (sim.SweepingVictory(Fraction.Separatists) == p &&
                    sim.SweepingVictory(Fraction.Republic) == null)
                Console.WriteLine("OK");
            else { Console.WriteLine("WRONG"); }
